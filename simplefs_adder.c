...
int filename_exists(FILE *image, const char *filename)
{
    dirent_t entry;
    /* TODO: STUDENT CODE START */
    /* An inode never stores a name. The root directory block is the only place
       that maps a name to an inode number, so a duplicate check means scanning
       every entry in block 4. */
    ...
    for (int i = 0; i < BLOCK_SIZE / (int)sizeof(dirent_t); i++) {
        ...
        if (strcmp(entry.name, filename) == 0) return 1;
    }
    /* TODO: STUDENT CODE END */
    return 0;
}
...
int find_free_directory_entry(FILE *image)
{
    dirent_t entry;
    /* TODO: STUDENT CODE START */
    /* Skip entries 0 and 1: they are "." and "..", which must never be reused. */
    ...
    for (int i = 2; i < BLOCK_SIZE / (int)sizeof(dirent_t); i++) {
        if (fread(&entry, sizeof(entry), 1, image) != 1) break;
        if (entry.inode_no == 0) return i;   /* first free slot */
    }
    /* TODO: STUDENT CODE END */
    return -1;
}
